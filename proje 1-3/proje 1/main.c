...
//    mevcut matrisi sıfırlar.
    int knrs[3] = {0};
    double rezerv1=0,rezerv2=0,rezerv3=0,toplamrezerv=0;
    kenarsayisi(knrs);
    double alan[3];
    double fiyat;
    double birimdeger;
    double plartfom1=0,plartfom2=0,plartfom3=0,sondaj1=0,sondaj2=0,sondaj3=0,toplamplatform=0,toplamsondaj=0;
    printf("platform fiyatı giriniz : ");
    scanf("%lf",&fiyat);
    printf("birim sondaj degerini giriniz : ");
    scanf("%lf",&birimdeger);
...
//                                    bölge bölge hesaplanan alanı en sonda for döngüyüsüyle toplama
    ...
    if(knrs[0]!=0){
            double rezerv1 = (alan[0] - toplam) * 10;
            double plartfom1 = alan[0] - toplam;
            double sondaj1 = alan[0] - toplam;
            toplamplatform=plartfom1+plartfom2+plartfom3;
            toplamsondaj=sondaj1+sondaj2+sondaj3;
            toplamrezerv=rezerv1+rezerv2+rezerv3;
            printf("1. seklin rezerv sayisi : %.2lf\n", rezerv1);
            printf("1. seklin birim kare sayisi : %.2lf\n", alan[0] - toplam);
            printf("1. seklin platform : %.2lf\n", plartfom1);
            printf("1. seklin sondaj sayisi : %.2lf\n", sondaj1);
    ...
    if(knrs[1]!=0){
           rezerv2=(alan[1]-toplam1)*10;
           plartfom2=alan[1]-toplam1;
           sondaj2=alan[1]-toplam1;
           toplamplatform=plartfom1+plartfom2+plartfom3;
           toplamsondaj=sondaj1+sondaj2+sondaj3;
           toplamrezerv=rezerv1+rezerv2+rezerv3;
           printf("2. seklin birim kare sayisi : %.2lf\n",alan[1]-toplam1);
           printf("2. seklin platform sayisi : %.2lf\n", plartfom2);
           printf("2. seklin sondaj sayisi : %.2lf\n", sondaj2);
       }
    glEnd();
    ...
    for (int i = 0; i < knrs[2]; i++) {
        ...
        double balan2=0;// bolge alani
           ...
           glEnd();
    if(knrs[2]!=0){
               rezerv3=(alan[2]-toplam2)*10;
               plartfom3=alan[2]-toplam2;
               sondaj3=alan[2]-toplam2;
               toplamplatform=plartfom1+plartfom2+plartfom3;
               toplamrezerv=rezerv1+rezerv2+rezerv3;
               toplamsondaj=sondaj1+sondaj2+sondaj3;
               printf("3. seklin birim kare sayisi : %.2lf\n",alan[2]-toplam2);
               printf("3. seklin platform : %.2lf\n", plartfom3);
               printf("3. seklin sondaj sayisi : %.2lf\n", sondaj3);
           }
           double maliyet=(toplamplatform*fiyat)+(toplamsondaj*birimdeger);
           printf("toplam rezerv=%.2lf\n",toplamrezerv);
           printf("toplam maliyet : %.2lf\n",maliyet);
           printf("kar miktarı : %.2lf\n",toplamrezerv-maliyet);
...
//curl işaretçisini başlatır.başarılıysa bu işlem yeni bir curl işlemini temsil eder
    if (curl) {
        curl_easy_setopt(curl, CURLOPT_URL, "https://gist.githubusercontent.com/ctnasu/b3ebca58a40d2bf3824f595c716ed8a6/raw/771113247e3465114b23d644f7314a44a1ee953b/coordinates");
//url yerleştirilir.
